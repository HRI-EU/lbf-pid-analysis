...
def summary_plot_pid(
    settings, results_by_heuristic, target_variable, filename, render
):
    ...
    """
    measures = [
        ["syn_iccs", "shd_iccs", "unq1_iccs", "unq2_iccs"],
        ["syn_sx", "shd_sx", "unq1_sx", "unq2_sx"],
        ["syn_sx_cor", "shd_sx_cor", "unq1_sx_cor", "unq2_sx_cor"],
        ["syn_syndisc", "shd_syndisc", "unq1_syndisc", "unq2_syndisc"],
        ["syn_norm_iccs", "syn_norm_sx", "syn_norm_sx_cor"],
    ]
    colors = [
    ...
    labels = [
        ...
        ["$I_{syn}^{CCS}/I$", "$I_{syn}^{SX^c}/I$", "$I_{syn}^{SD}/I$"],
    ]
...
def make_summary_plot(  # pylint: disable=too-many-locals
    ...
    if correlate_measure is not None:
        ...
        if np.isnan(
            correlations
        ).any():  # replace NaNs by zeros for proper plotting
...
def get_trial_file_names(foldername):
    ...
    logging.info(
        "Reading trial data from folder: %s, found %d trials: %s",
        foldername,
        len(trial_files),
        trial_files[0],
    )
...
def load_experiment_data(filename, load_field=False):
    ...
    if load_field:
        field = np.load(
            filename.replace("game_data", "field").replace("csv", "npy")
        )
        logging.debug(field)
        return df, field
    return df
...
def get_experiment_performance(  # pylint: disable=too-many-locals
    ...
    ax[0, 0].scatter(
        np.where(food_value_0)[0], food_value_0[food_value_0 > 0], c="b"
    )
    ...
    ax[0, 2].axis("off")
    ax[1, 0].scatter(
        np.where(food_value_1)[0], food_value_1[food_value_1 > 0], c="r"
    )
    ...
    return {
        ...
        "any_food_collected": np.logical_or(
            food_collected_0, food_collected_1
        ).astype(
            int
...
def encode_source_variables(experiment_data, source_type, source_encoding):
    ...
    """
    logging.info(
        "Using agent %s as sources/inputs to PID analysis (encoding used: %s)",
        source_type,
        source_encoding,
    )
    ...
    if source_type == "actions":
        ...
        raise RuntimeError(
            "Unknown source variable %s"  # pylint: disable=W1201,C0209
            % source_type
        )
...
def encode_target_variable(performance, target_variable):
    ...
    """
    logging.info("Using %s as target of PID analysis", target_variable)
    ...
    if (
        ...
        if len(np.unique(performance["cooperative_actions"])) == 1:
            target = np.zeros(
                len(performance["cooperative_actions"]), dtype=int
            )
        else:
            n_bins = 2
        ...
        target_variable == "mean_food_value_collected"
    ):  # mean collected food value
        if len(np.unique(performance["mean_food_value_collected"])) == 1:
            target = np.zeros(
                len(performance["mean_food_value_collected"]), dtype=int
            )
        else:
        ...
        if len(np.unique(performance["total_food_value_collected"])) == 1:
            target = np.zeros(
                len(performance["total_food_value_collected"]), dtype=int
            )
        else:
            n_bins = len(cfg.actions)
            target = utils.discretise(
        ...
        if len(np.unique(performance["n_collections"])) == 1:
            ...
            n_bins = 3
            target = utils.discretise(
                performance["n_collections"], numBins=n_bins
            )
        ...
        target_variable == "food_type"
    ):  # no. food items collected in each step, 0-2
        if len(np.unique(performance["food_type_collected"])) == 1:
            target = np.zeros(
                len(performance["food_type_collected"]), dtype=int
            )
        else:
            n_bins = 3
            target = utils.discretise(
        ...
        if len(np.unique(performance["n_collections_agent_0"])) == 1:
            target = np.zeros(
                len(performance["n_collections_agent_0"]), dtype=int
            )
        else:
            n_bins = 2
            target = utils.discretise(
        ...
        if len(np.unique(performance["n_collections_agent_1"])) == 1:
            target = np.zeros(
                len(performance["n_collections_agent_1"]), dtype=int
            )
        else:
            n_bins = 2
            target = utils.discretise(
        ...
        if len(np.unique(performance["any_food_collected"])) == 1:
            ...
            n_bins = 2
            target = utils.discretise(
                performance["any_food_collected"], numBins=n_bins
            )
            assert len(np.unique(target)) <= 2
            assert np.max(target) <= 1
            assert np.min(target) >= 0
...
def plot_variable_distributions(
    ...
    if outpath is not None:
        if figurename is None:
            raise RuntimeError(
                "Provide a figurename to plot the variable distribution"
            )
...
def estimate_joint_mutual_information(source_0, source_1, target, n_perm, seed):
    ...
    logging.info("Joint MI: %.4f (sign=%s, p=%.4f)", mi, sign_mi, p)
...
def main(path, settings_path, folders, target_variable, render=False):
    ...
    for folder_number in folders:  # each folder holds one experiment
        ...
        for trial in range(experiment_settings["experiment"]["ntrials"]):
            print("\n")
            logging.info(
                "Analyzing trial %d (folder %s, heuristic %s, coop=%.2f)",
                trial,
                folder_number,
                heuristic,
                float(experiment_settings["environment"]["coop"]),
            ...
            if trial < 5:
                performance = get_experiment_performance(
                    ...
                    experiment_settings,
                    trial,
                    outpath=outpath.joinpath("trial_data"),
                    render=render,
                )
            else:
                performance = get_experiment_performance(
            ...
            results_all = _append_trial_performance(
                results_all, heuristic, performance
            )
            ...
            if np.all(source_0 == source_0[0]) or np.all(
                source_1 == source_1[0]
            ):
                logging.info("Found constant value in one of the sources")

            # I_ccs estimation
            pid_iccs = estimate_i_ccs_pid_dit(source_0, source_1, target)
            results_all = _append_pid_results(
                pid_iccs, "iccs", results_all, heuristic
            )

            # SxPID estimation - uncorrected
            pid_sx = estimate_sx_pid(
                source_0, source_1, target, correction=False
            )
            results_all = _append_pid_results(
                pid_sx, "sx", results_all, heuristic
            )

            # SxPID estimation - corrected
            pid_sx_cor = estimate_sx_pid(
                source_0, source_1, target, correction=True
            )
            results_all = _append_pid_results(
                pid_sx_cor, "sx_cor", results_all, heuristic
            )

            # Syndisc PID estimation
            pid_syndisc = estimate_pid_syndisc(source_0, source_1, target)
            results_all = _append_pid_results(
                pid_syndisc, "syndisc", results_all, heuristic
            )
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="PID estimation for LBF experiments."
    )
